...
p_pms<-filter(res_rh_temp) %>%
  filter(grepl("PMS5003",sensor)) %>%
  ggplot()+    
  geom_boxplot(aes(x = rh_median,y=spearman,group=rh_median), size=0.5, fatten=0.8)+
      geom_line(aes(x = rh_median,y=spearman, group = sensor, color = sensor,alpha=0.9))+

 ylim(c(0,1.2))+
  facet_grid(~source)+theme_bw()+ylab("spearman") + ggtitle("PMS5003") + xlab("Relative humidity (%)")+theme(legend.position = "none", axis.title.x=element_blank(), axis.title.y=element_blank(), plot.title = element_text(hjust = 0.5))
p_pms 
p_opcr1<-filter(res_rh_temp) %>%
  filter(grepl("OPCR1",sensor)) %>%
   ggplot()+    
 geom_boxplot(aes(x = rh_median,y=spearman,group=rh_median), size=0.5, fatten=0.8)+
      geom_line(aes(x = rh_median,y=spearman, group = sensor, color = sensor,alpha=0.9))+
  facet_grid(~source)+theme_bw()+ylab("spearman") + ggtitle("OPCR1") + xlab("Relative humidity (%)")+theme(legend.position = "none", axis.title.y=element_blank(), plot.title = element_text(hjust = 0.5))

p_sps<-filter(res_rh_temp) %>%
  filter(grepl("SPS030",sensor)) %>%
   ggplot()+    
 geom_boxplot(aes(x = rh_median,y=spearman,group=rh_median), size=0.5, fatten=0.8)+
      geom_line(aes(x = rh_median,y=spearman, group = sensor, color = sensor,alpha=0.9))+
   ylim(c(0,1.2))+
  facet_grid(~source)+theme_bw()+ylab("spearman") + ggtitle("SPS030")+xlab("Relative humidity (%)")+theme(legend.position = "none", axis.title.x=element_blank(), axis.title.y=element_blank(), plot.title = element_text(hjust = 0.5))


p_sds<-filter(res_rh_temp) %>%
  filter(grepl("SDS018",sensor)) %>%
   ggplot()+    
  geom_boxplot(aes(x = rh_median,y=spearman,group=rh_median), size=0.5, fatten=0.8)+
    geom_line(aes(x = rh_median,y=spearman, group = sensor, color = sensor,alpha=0.9))+
   ylim(c(0,1.2))+
  facet_grid(~source)+theme_bw()+ylab("spearman") + ggtitle("SDS018") + xlab("Relative humidity (%)")+theme(legend.position = "none", axis.title.x=element_blank(), axis.title.y=element_blank(), plot.title = element_text(hjust = 0.5))

p_hpma<-filter(res_rh_temp, spearman_p_value<=0.05) %>%
  filter(grepl("HPMA115S0",sensor)) %>%
  ggplot()+    
  geom_boxplot(aes(x = rh_median,y=spearman,group=rh_median), size=0.5, fatten=0.8)+
      geom_line(aes(x = rh_median,y=spearman, group = sensor, color = sensor,alpha=0.9))+
   ylim(c(0,1.2))+
  facet_grid(~source)+theme_bw()+ylab("spearman") + ggtitle("HPMA115S0") + xlab("Relative humidity (%)")+theme(legend.position = "none", axis.title.x=element_blank(), axis.title.y=element_blank(), plot.title = element_text(hjust = 0.5))
